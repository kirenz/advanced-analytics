...
The function `ggscatmat` from the package `GGally` creates a matrix with scatterplots, densities and correlations for numeric columns. In our code, we enter the dataset `housing`, choose columns 6 to 9, a color column for our categorical variable `ocean_proximity`, and an alpha level of 0.8 (for transparency).

```{r }

ggscatmat(housing, columns = 6:9, color="ocean_proximity", alpha=0.8)
...
Let's assume we would know that the median income is a very important attribute to predict median housing prices. Therefore, we would want to create a training and test set using stratified sampling. A *stratum* (plural strata) refers to a subset (part) of the population (entire collection of items under consideration) which is being sampled. Take a look at \@ref(fig:hist-med-income)
...
```{r plot-ca-prices, fig.cap="California housing prices", out.width='80%'}
...
price_pred <- 
  ...
  mutate(price_truth = housing$median_house_value)
